...
<xsl:stylesheet version="2.0" xmlns="http://www.opengis.net/kml/2.2"
	...
	<xsl:template
		...
		<xsl:if test="$coordinates">
			<Placemark>
               ...
               <tr><td>Name</td><td><a href="#" onclick="var w=window.open(']]>
					<xsl:value-of select="'wfsFeaturePopup.do?url='"/>
					<xsl:value-of select="$mineNameHrefLink"/><![CDATA[','AboutWin','toolbar=no, menubar=no,location=no,resizable=yes,scrollbars=yes,statusbar=no,height=450,width=850');w.focus();return false;">]]>
					<xsl:value-of
						select="./er:specification/er:Mine/gml:name[@codeSpace='http://www.ietf.org/rfc/rfc2616']"/><![CDATA[</a>]]><![CDATA[</td></tr><tr><td>Preferred Name</td><td>]]>
	...
	<xsl:template match="gml:Point">
		...
		<Point>
			...
			<coordinates>
				<xsl:choose>
					<xsl:when
						test="starts-with(@srsName,'http://www.opengis.net/gml/srs/epsg.xml#4283')">
	...
	<xsl:template match="gml:MultiCurve">
		...
		<xsl:variable name="coordinates">
			<xsl:value-of
				select="./gml:curveMembers/gml:Curve/gml:segments/gml:LineStringSegment/gml:posList | ./gml:curveMember/gml:LineString/gml:posList"
			/>
		</xsl:variable>
	...
	<xsl:template name="displayUrnResolverLink">
		...
		-->
		<![CDATA[</td></tr><tr><td>]]><xsl:value-of select="$tableRowLabel"
			/><![CDATA[</td><td><a href="#" onclick="var w=window.open(']]><xsl:value-of
			select="$tableRowValue"
			/><![CDATA[','AboutWin','toolbar=no, menubar=no,location=no,resizable=yes,scrollbars=yes,statusbar=no,height=450,width=850');w.focus();return false;">]]><xsl:value-of
	...
	<xsl:template name="displayUrnResolverLinkWithoutHTTP">
		...
		<![CDATA[</td><td><a href="#" onclick="var w=window.open(']]><xsl:value-of
			select="'wfsFeaturePopup.do?url='"/><xsl:value-of select="$tableRowValue"
			/><![CDATA[','AboutWin','toolbar=no, menubar=no,location=no,resizable=yes,scrollbars=yes,statusbar=no,height=450,width=850');w.focus();return false;">]]><xsl:value-of
